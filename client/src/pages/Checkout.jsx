...
const Checkout = () => {
    ...
    useEffect(() => {
        ...
        const fetchCustomerDetails = async () => {
            try {
                ...
                const res = await axios.get("http://localhost:5002/api/auth/customers/me", {
    ...
    const handleConfirmOrder = async () => {
        ...
        try {
            ...
            const response = await axios.post("http://localhost:5005/api/orders/confirm", orderData, {
            ...
            Swal.fire({
                ...
                const selectedRating = document.querySelector('[data-selected]') 
                    ? document.querySelector('[data-selected]').getAttribute('data-selected') 
                    : null;
                ...
                if (selectedRating) {
                    ...
                    await axios.put(`http://localhost:5005/api/orders/${orderId}/rating`, { rating, review }, {
                        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
                    });
                }
    ...
    return (
        ...
        <br/>
        <a href="/cart" className="flex items-center font-roboto hover:text-orange-500 font-bold"><ArrowLeftIcon className="h-6 w-6 text-orange-500 mr-2" />Back to Cart</a>
        <br/>
        <div className="flex items-center justify-center">
            ...
            <div className="bg-white p-6 shadow-lg rounded-lg">
                ...
                <p><strong>Address:</strong> {customer.address || "Loading..."}</p>
                <p><strong>Phone:</strong> {customer.phone || "Loading..."}</p><br/>

                <h2 className="text-xl font-bold mt-4 mb-2">Ordered Items</h2>
                {cart.items.length > 0 ? (
                    cart.items.map((item, index) => (
                        <div key={index} className="border-b py-2">
                            <p><strong>{item.name}</strong> ({item.size}) - Rs. {item.price * item.quantity}</p>
                        </div>
                    ))
                ) : (
                ...
                <h2 className="text-3xl font-bold mt-4">Total Payable: Rs. {cart.totalPayable}.00</h2><br/>
                ...
                <Button text="Confirm Order" className="mt-4" onClick={handleConfirmOrder} />
            </div><br/>
        </div>
    );
};
