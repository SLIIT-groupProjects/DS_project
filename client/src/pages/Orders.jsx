...
const Orders = () => {
    ...
    return (
        ...
        <br/>
        <div className="flex items-center justify-center">
            <h1 className="font-roboto text-3xl font-bold mb-4 mx-4">YOUR ORDERS</h1>
            ...
            {orders.length === 0 ? (
                ...
                <div className="space-y-4">
                    {orders.map((order) => (
                        <div key={order._id} className="relative bg-white shadow-lg rounded-lg p-4">
                            <p><strong>Address:</strong> <span className="ml-[100px]">{order.address}</span></p>
                            <p><strong>Phone:</strong> <span className="ml-[112px]">{order.phone}</span></p>
                            <p><strong>Total Payable:</strong> <span className="ml-[60px]">Rs. {order.totalPayable}.00</span></p>
                            <p><strong>Status:</strong> <span className="ml-[114px]">{order.status}</span></p>
                            <p><strong>Delivery Option:</strong> <span className="ml-[41px]">{order.deliveryOption}</span></p>
                            {order.deliveryOption === "schedule" && order.scheduledTime && (
                                <p><strong>Scheduled Time:</strong> <span className="ml-[41px]">{new Date(order.scheduledTime).toLocaleString()}</span></p>
                            )}

                            <div className="mt-4">
                                <strong className="text-orange-500">Rating:</strong> <span className="ml-[108px]">{order.rating ? renderStars(order.rating) : " Not Rated"}</span> <br/>
                                <strong className="text-orange-500">Review:</strong> <span className="ml-[105px]">{order.review ? order.review : "No review yet."}</span>
                            </div>
